...
func (l *UserCenterPlayerLinker) LinkPlayer(ctx context.Context, accountID, playerID string) error {
	...
	payload, err := proto.Marshal(&internalpb.LinkPlayerRequest{AccountId: accountID, PlayerId: playerID})
	if err != nil {
		return err
	}
	response, err := client.Request(ctx, &internalpb.InternalEnvelope{TargetService: internalpb.ServiceType_SERVICE_TYPE_USERCENTER, MessageId: uint32(internalpb.UserCenterMessageId_USER_CENTER_MESSAGE_ID_LINK_PLAYER_REQUEST), Payload: payload})
	if err != nil {
		return err
	}
