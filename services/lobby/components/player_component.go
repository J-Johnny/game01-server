...
func (c *PlayerComponent) RegisterInternal(router *streaming.Router) {
	router.Register(internalpb.ServiceType_SERVICE_TYPE_LOBBY,
		uint32(internalpb.LobbyMessageId_LOBBY_MESSAGE_ID_ENSURE_PLAYER_REQUEST),
		streaming.MessageHandlerFunc(c.ensurePlayer))

	router.Register(internalpb.ServiceType_SERVICE_TYPE_LOBBY,
		uint32(internalpb.LobbyMessageId_LOBBY_MESSAGE_ID_SETTLEMENT_REQUEST),
		streaming.MessageHandlerFunc(c.settlement))

	router.Register(internalpb.ServiceType_SERVICE_TYPE_LOBBY,
		uint32(internalpb.InternalMessageId_INTERNAL_MESSAGE_ID_RESTORE_PLAYER_STATE_REQUEST),
		streaming.MessageHandlerFunc(c.restore))
}
...
func (c *PlayerComponent) ensurePlayer(ctx context.Context, _ streaming.Peer, envelope *internalpb.InternalEnvelope) (*streaming.MessageResult, error) {
	...
	if errors.Is(err, domain.ErrPlayerNotFound) {
		...
		create := func(transactionContext context.Context) error {
			...
			return c.snapshot.Save(transactionContext, domain.Snapshot{
				Player:        candidate,
	...
	return &streaming.MessageResult{
		MessageID: uint32(internalpb.LobbyMessageId_LOBBY_MESSAGE_ID_ENSURE_PLAYER_RESPONSE),
		Message:   &internalpb.EnsurePlayerResponse{AccountId: accountID, PlayerId: player.ID, Created: created}}, nil
}
...
func (c *PlayerComponent) settlement(ctx context.Context, _ streaming.Peer, envelope *internalpb.InternalEnvelope) (*streaming.MessageResult, error) {
	...
	if existing, err := c.ledger.FindBySettlementID(ctx, settlement.ID); err == nil {
		if existing.PlayerID != settlement.PlayerID || existing.AssetType != settlement.AssetType ||
			existing.Delta != settlement.Delta || existing.Reason != settlement.Reason || existing.Source != settlement.Source {
